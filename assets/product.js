...
const initializePurchaseOptions = () => {
      ...
      if (input.id === 'purchase-options-one-time') {
        ...
        if (input.closest('#product-template')) {
          ...
          url.searchParams.set(
            'variant',
            wrapper.closest('.product-content').querySelector('[name="id"]').value
          );
          url.searchParams.set(
            'selling_plan',
  ...
  const selectDeliverEvery = wrapper.querySelector('#options-select-purchase-delivery');
...
window.handleProductOptionChange = async input => {
  ...
  if (variantSelected) {
    input.closest('form').querySelector('[name="id"]').value = variantSelected.id;
    ...
    const purchaseOptionsWrapper = document.querySelector('#purchase-options-product');
    ...
    if (purchaseOptionsWrapper) {
      ...
      ).textContent = Shopify.formatMoney(variantSelected.selling_plan_allocations[0].price);
    }
  ...
  if (input.closest('.size-buttons')) {
    input.closest('.product-option-wrapper').querySelector('.size-buttons-title span').textContent =
      input.value;
  }
};
...
const initializeProductGallery = () => {
  document.querySelectorAll('.product-gallery:not(.init)').forEach(gallery => {
    ...
    if (window.matchMedia('(min-width: 992px)').matches) {
      ...
      galleryWrapper.style.top = `${navbarHeight + announcementBarHeight + 20}px`;
...
window.addEventListener('updated.shopiweb.collection', initializeProductGallery);
...
const initializeUpsellModal = () => {
  ...
  document.querySelectorAll('#modal-upsell-product .btn-atc').forEach(btn => {
    btn.addEventListener('click', () => {
      const footerBtn = document.querySelector('#modal-upsell-product .modal-footer .btn');

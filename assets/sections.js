...
const stickyAnnouncementBarInit = () => {
  ...
  navbar.style.top = document.querySelector('.sticky-announcement-bar').clientHeight + 'px';
};
    ...
    const mySplide = new Splide(element, {
      ...
      breakpoints: {
        0: {
          ...
          padding: element.dataset.breakpointXsPartial === 'true' ? { right: '16%' } : 0
        },
        ...
        productListItems += `
                    <li class="product-item splide__slide p-3 p-lg-4 mb-4">
                        ...
                        <div class="product-item-vendor mb-2 text-muted small text-uppercase" ${
                          section.dataset.productVendor === 'true' ? '' : 'hidden'
                        }>${product.vendor}</div>
                        ...
                        <p class="product-item-price mb-4">
                            ...
                            <span class="product-item-price-final" ${
                              product.compare_at_price > product.price ? 'hidden' : ''
                            }>
                                <span ${product.price_varies ? '' : 'hidden'}>
                                    ${section.dataset.textPriceFrom}
                                </span>
...
const initializeStickyAddToCart = async () => {
  ...
  const threshold =
    document.querySelector('.product-content form[action*="/cart/add"]').getBoundingClientRect()
...
const initializePopupNewsletter = async () => {
  ...
  window.createNewCookie('shopiweb-newsletter-popup', true, Number(modal.dataset.daysToWait));
...
const initializeStoriesAnimated = () => {
  ...
  document.querySelectorAll('.animated-stories-link').forEach(link => {
    ...
    if (new Date().getTime() > Number(localStorage.getItem(localStorageKeyTime)) + 8.64e7) {
      storiesRead = [];
    }
...
const initializeQuiz = () => {
  document.querySelectorAll('section.quiz').forEach(quiz => {
      ...
      btn.addEventListener('click', () => {
        ...
        setTimeout(() => {
          ...
          progressBar.style.width =
            ((Number(block.dataset.question) + 1) / Number(progressBar.dataset.total)) * 100 + '%';
          ...
          if (Number(block.dataset.question) === Number(progressBar.dataset.total)) {
            ...
            results.querySelectorAll('.product-listing').forEach(prouductList => {
    ...
    quiz.querySelector('.btn-quiz-prev').addEventListener('click', () => {
      ...
      progressBar.style.width =
        ((Number(currentBlock.dataset.question) - 2) / Number(progressBar.dataset.total)) * 100 +
        '%';
      ...
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          ...
          if (circle.getAttribute('data-animated') !== 'true') {
            ...
            const speed = parseInt(circle.getAttribute('data-speed'), 10) * 1000;
...
const updateCircle = (circle, speed, progressEnd) => {
  ...
  const update = () => {
    if (progressCurrent < progressEnd) {
      ...
      } ${3.6 * progressCurrent}deg, ${circle.dataset.circleInactiveColor} 0deg)`;
      if (progressCurrent < progressEnd) {
...
document.addEventListener('DOMContentLoaded', function () {
  if (window.collageDynamiqueSectionId) {
    var gridSelector = '#collage-dynamique-' + window.collageDynamiqueSectionId + ' .row';

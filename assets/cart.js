...
const initializeCartGoalFreeProduct = async () => {
  ...
  let isAlreadyInCart = [...document.querySelectorAll('[data-cart-line-item]')].some(
    e => e.dataset.productHandle === handle
  );
...
window.fetchShippingRates = async btn => {
  ...
  try {
    ...
    if (data.shipping_rates.length) {
      const list = data.shipping_rates
        .map(
          rate =>
            `<li><strong>${rate.presentment_name}</strong>: ${rate.price} ${rate.currency}</li>`
        )
        .join('');
      modal.querySelector('#shipping-rates-modal-alert-success').innerHTML =
        `<ul class="ps-5 mb-0">${list}</ul>`;
      modal.querySelector('#shipping-rates-modal-alert-success').removeAttribute('hidden');
    } else {
      modal.querySelector('#shipping-rates-modal-alert-warning').innerHTML =
        `<p class="mb-0">${modal.dataset.textNoResults}</p>`;
      modal.querySelector('#shipping-rates-modal-alert-warning').removeAttribute('hidden');
    }
  } catch (err) {
    const list = Object.entries(err)
